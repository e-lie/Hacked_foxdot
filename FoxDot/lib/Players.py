...
from .Patterns import Pattern, PGroup, asStream, get_first_item, modi, pattern_depth, get_expanded_len, GeneratorPattern, group_modi, Rest
...
class Player(Repeatable):
    ...
    keywords = (
        'degree', 'oct', 'freq', 'dur', 'delay', 'buf', 'blur', 'amplify', 'scale', 'bpm', 'sample', 'sdb', "env"
    )
    ...
    def reset(self):
        ...
        for key in Player.Attributes():
            if key not in ("scale", "dur", "sus", "blur", "amp", "amplify", "degree", "oct", "bpm"):
                setattr(self, key, 0)
            reset.append(key)
        ...
        self.sus = 0.5 if self.synthdef == SamplePlayer else 1  # Sustain & Legato
        self.blur = 1
